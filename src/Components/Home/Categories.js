import './Styles/categories.css';
import Produits from '../../Backends/Produits';
import { Alerts } from '../Communs/Alerts';
import { useEffect, useState } from 'react';
import { BiCategoryAlt } from 'react-icons/bi';

export default function Categories(){
    const urlBase= 'https://insta-api-api.0vxq7h.easypanel.host/';

    const [prods, setProds] =  useState([]);
    const [Alert, setAlert]=useState({Etat: false, Titre: '', Type: '', Message: ''});

    useEffect(()=>{
        const produits= new Produits(urlBase);
        produits.getProducts().then(p=>{
            setProds(p);
        }).catch(error=>{
            setAlert({Etat: true, Titre: 'Error list all products', Type: 'ERROR', Message: error.message});
            console.log(error);
        });
    }, []);

    function category(produit){
        let cats= [];
        for (const item of produit){
            const isExist = (element) => element.id === item.category.id;
            const index= cats.findIndex(isExist);
            if (index === -1){
                cats.push({id: item.category.id, name: item.category.name, image: item.image, totalProduit: 1});
            } else {
                cats[index].totalProduit= cats[index].totalProduit + 1; 
            }
        }
        return cats;
    }

    function onFermerAlert(){
        setAlert({Etat: false});
    }

    return(
        <>
            <div className='mx-5 my-5'>
                <h2 className='w-100' style={{borderBottom: '1px solid var(--bs-primary)'}}><BiCategoryAlt className="me-2 text-dark" style={{fontSize:'40px'}}/>CATÉGORIES</h2>
                <div className='d-flex justify-content-between flex-wrap my-4' style={{width: '100%'}}>
                    {category(prods).map(item=>(
                        <div key={item.id} className='bouton d-flex align-items-center border-top border-start border-3 border-primary' style={{cursor: 'pointer'}}>
                            <div><img src={item.image} alt="" style={{height: '100px', objectFit: 'contain'}} /></div>
                            <div className='btnText d-flex align-items-center flex-column py-4'>
                                <h5 className='btnTextCat my-0 w-100 text-center'>{item.name}</h5>
                                <p className='my-0 w-100 text-center'>({item.totalProduit}) Produits</p>
                            </div>
                        </div>    
                    ))}
                </div>
            </div>

            <Alerts Etat={Alert.Etat} Type={Alert.Type} Titre={Alert.Titre}  Message={Alert.Message} onFermer= {onFermerAlert}/>
        </>
    )
}