...
export default function FiltreCouleur() {
    ...
    return (
        <>
            <div className='my-4'>
                ...
                <div className='px-3 pt-2 border d-flex align-items-start'>
                    <Form className='d-flex flex-column w-100 my-3'>
                        <Form.Check className='fw-bold my-1' type='checkbox' id= 'id-0' label= 'Toutes les couleurs'/>
                        {Couleur(prods).map(item=>(    
                            <div className='d-flex my-1'>
                                <Form.Check type='checkbox' id={`id-${item.id}`} label= {<span style={{color: item.code}}>{item.name}</span>} />
